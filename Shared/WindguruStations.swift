...
// MARK: - Locally grown sparkline

/// Windguru's station history query needs a logged-in session, so the sparkline
/// is grown here instead: every widget refresh appends the current reading.
/// Points are keyed by the station's own measurement time, so refetching an
/// offline station doesn't flatline the chart with duplicates. The 3 h window
/// holds at best ~36 points (5-minute refreshes), usually fewer — WidgetKit's
/// budget grants refreshes when it pleases — so the chart is coarser than the
/// Grafana widgets' server-side 1-minute series. Two widgets on the same
/// station race on this key; both write the same point, so the result stands.
enum StationHistory {
    ...
    private static let window: TimeInterval = 3 * 3600

    private static func key(_ stationID: String) -> String { "stationHistory_\(stationID)" }

    /// Record the reading and return the current window as sparkline values.
    static func append(_ r: Windguru.StationReading, for stationID: String) -> [Double?] {
        var points = load(for: stationID)
        // Strictly newer only: an equal timestamp is the same reading again
        // (station offline), an older one would break the chart's time order.
        if let at = r.at, let wind = r.wind,
           points.last.map({ at > $0.t }) ?? true {
            points.append(Point(t: at, v: wind))
        }
        points.removeAll { $0.t < Date().addingTimeInterval(-window) }
        if let data = try? JSONEncoder().encode(points) {
            UserDefaults.standard.set(data, forKey: key(stationID))
        }
        // The Sparkline view needs at least two values to draw anything.
        return points.count >= 2 ? points.map { $0.v } : []
    }

    static func load(for stationID: String) -> [Point] {
        guard let data = UserDefaults.standard.data(forKey: key(stationID)) else { return [] }
